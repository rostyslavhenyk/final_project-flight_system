...
internal fun bookPassengersModel(
    ...
    return mapOf(
        ...
        "membershipValue" to membershipValue(logged),
        "membershipFilled" to (logged.loggedIn && session != null),
        "continueSeatsHref" to bookingHref("/book/seats", queryParams),
    )
}

private fun membershipValue(logged: LoggedInState): String =
    if (logged.loggedIn && logged.session != null) {
        String.format(Locale.UK, MEMBER_NUMBER_FORMAT, logged.session.id)
    } else {
        ""
    }
